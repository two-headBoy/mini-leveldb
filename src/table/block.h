...
class Block {
    ...
    class Iterator {
       ...
       private:
        ...
        const std::vector<uint32_t>&
            restarts_;  // 引用 Block 内部数组，生命周期绑定
        ...
        //  读三元组(shared/unshared/vlen) → key_.resize(shared).append(差量) →
        //  value_ 切片 → current_ 推进整条 entry
   ...
   private:
    ...
    std::vector<uint32_t>
        restarts_;  // restart offsets（在 data_ 之后，构造时倒推得到）
};
