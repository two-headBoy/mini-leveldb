...
// entry 格式：[varint shared][varint unshared][varint value_len][key差量][value]
// 每 16 条埋 restart point（shared=0 存全量 key），读端据此二分定位
// 块尾：restart offsets 数组 + 4B count（读端倒推 count 找数组起点）
class BlockBuilder {
public:
    BlockBuilder() { Reset(); }
